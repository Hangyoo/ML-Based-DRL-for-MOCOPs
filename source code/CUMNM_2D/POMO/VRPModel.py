...
class VRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        # shape: (batch, 3*problem+1, embedding)
    ...
    def forward(self, state):
        ...
        if state.attribute_kp:
            ...
            probs = self.decoder(self.encoded_graph, state.load, state.capacity, state.time, ninf_mask=state.ninf_mask)
            ...
            if state.selected_count == 0:
                ...
                probs = self.decoder(encoded_last_node, state.load, state.capacity, state.time, ninf_mask=state.ninf_mask)  # decoder 返回每个点的概率值，已经选过的prob值为0
        ...
        elif state.attribute_c or state.attribute_tw:
            if state.selected_count == 0:  # First Move, depot
                ...
                probs = self.decoder(encoded_last_node, state.load, state.capacity, state.time, ninf_mask=state.ninf_mask) # 原始程序 state.ninf_mask
...
class VRP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.hyper_output_dim = 5 * self.embd_dim # 根据权重向量扩展出的网络权重
        ...
        self.hyper_Wq_first = nn.Linear(self.embd_dim, embedding_dim * head_num * qkv_dim, bias=False)  # 2 --> 128*8*16
        self.hyper_Wq_last = nn.Linear(self.embd_dim, (embedding_dim+3) * head_num * qkv_dim, bias=False)  # 2 --> 16384  # 原始程序 +4
        ...
        self.Wq_last = nn.Linear(embedding_dim+3, head_num * qkv_dim, bias=False)  # 原始程序 +4
        self.Wk = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wv = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
    ...
    def assign(self, pref): # pref: tensor([0.772,0.228])
        ...
        self.Wq_first_para = self.hyper_Wq_first(mid_embd[:self.embd_dim]).reshape(embedding_dim, head_num * qkv_dim) # 2 --> 128*8*16
        self.Wq_last_para = self.hyper_Wq_last(mid_embd[self.embd_dim:2 * self.embd_dim]).reshape(head_num * qkv_dim, embedding_dim+3) # 2 --> 128*128  # 原始程序 +4
        self.Wk_para = self.hyper_Wk(mid_embd[2 * self.embd_dim: 3 * self.embd_dim]).reshape(embedding_dim, head_num * qkv_dim) # 2 --> 128*128
        self.Wv_para = self.hyper_Wv(mid_embd[3 * self.embd_dim: 4 * self.embd_dim]).reshape(embedding_dim, head_num * qkv_dim) # 2 --> 128*128
        self.multi_head_combine_para = self.hyper_multi_head_combine(mid_embd[4 * self.embd_dim: 5 * self.embd_dim]).reshape(head_num * qkv_dim, embedding_dim) # 128*128
    ...
    def forward(self, encoded_last_node, load, capacity, time, ninf_mask): # encoded_last_node, load, time,length, route_open, ninf_mask
        '''
