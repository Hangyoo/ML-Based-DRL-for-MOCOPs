...
class VRPTrainer:
    def __init__(self,
        ...
        #Parallel
            ...
            #self.sub_model = torch.nn.DataParallel(self.sub_model.to(device), device_ids=[4,5])
            #self.sub_model = torch.nn.DataParallel(self.sub_model)
    ...
    def _train_one_batch(self, batch_size):
        ...
        if self.env.attribute_c:
            get_inner_model(self.sub_model).decoder_CVRP.assign(pref)
        elif self.env.attribute_tw:
            get_inner_model(self.sub_model).decoder_VRPTW.assign(pref)
        elif self.env.attribute_kp:
            get_inner_model(self.sub_model).decoder_KP.assign(pref)
        elif self.env.attribute_tsp:
            get_inner_model(self.sub_model).decoder_TSP.assign(pref)
        else:
            raise NotImplementedError

        get_inner_model(self.sub_model).pre_forward(reset_state)
        ...
        while not done:
            if self.env.attribute_kp:
                selected, prob = get_inner_model(self.sub_model)(state)
                # shape: (batch, pomo)
