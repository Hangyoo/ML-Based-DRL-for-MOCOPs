...
tester_params = {
    ...
    'model_load': {
        'path': 'Trained_models/50',  # 50_finetune directory path of pre-trained model and log files saved.
        'epoch': 12500,  # epoch version of pre-trained model to laod.
    },
    'test_episodes': 1,
    'test_batch_size': 1,
...
def main(n_sols = 101,idx=None):
    ...
    if env_params['instance_type'] == 'Random':
        np.savetxt(f"result/{subfile}/tsp_real/{env_params['instance_type']}{problem_size}_{idx}.txt", sols)
        with open(f"result/{subfile}/tsp_real/TIME_{env_params['instance_type']}{problem_size}_{idx}.txt", 'w') as f:
            f.write(str(total_time))
    else:
        np.savetxt(f"result/{subfile}/tsp_real/{env_params['instance_type']}{problem_size}_each_decoder_real.txt", sols)
        with open(f"result/{subfile}/tsp_real/TIME_{env_params['instance_type']}{problem_size}_each_decoder_real.txt", 'w') as f:
            f.write(str(total_time))
    ...
    plt.savefig(f'result/{subfile}/tsp_real/{ty}{item}_each_decoder_real.png')
    plt.clf()
    # plt.show()
