...
for ty in ['A', 'P', 'B', 'E','M','X']: # E
    ...
    for instance_name in [f for f in os.listdir(PATH) if (f.endswith('.csv') and f[-5] != 'l')]:
        ...
        def main(n_sols = 101,idx=None):
            ...
            np.savetxt(f"result/{subfile}/cvrp_real/{ty}/{instance_name.split('.')[0]}_each_decoder.txt", sols)
            with open(f"result/{subfile}/cvrp_real/{ty}/TIME_{instance_name.split('.')[0]}_each_decoder.txt", 'w') as f:
            ...
            plt.savefig(f'result/{subfile}/cvrp_real/{ty}/{instance_name.split(".")[0]}_each_decoder.png')
            plt.clf()
