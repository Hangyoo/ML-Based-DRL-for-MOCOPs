...
from Meta_Transformer_2D_reptile.VRProblemDef import get_random_problems_mixed, augment_xy_data_by_8_fold, augment_xy_data_by_64_fold, augment_xy_data_by_8_fold_tsp, augment_xy_data_by_64_fold_tsp
...
class VRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if not self.FLAG__use_saved_problems and (self.benchmark_instance == False):
            # 随机产生算例
            depot_xy, node_xy, node_demand, node_earlyTW, node_lateTW, node_servicetime, tsp_problems, kp_problems, type = get_random_problems_mixed(
                batch_size, self.problem_size, self.problem_type)
        elif self.benchmark_instance:
            # 使用benchmark算例
            self.batch_size = 1
            depot_xy, node_xy, node_demand, node_earlyTW, node_lateTW, node_servicetime, tsp_problems, kp_problems, type = get_random_problems_mixed(
                1, self.problem_size, self.problem_type)
            if self.problem_type == 'MOTSP':
                ...
                depot_xy_benchmark, node_xy_benchmark, node_demand_benchmark = read_benchmark_cvrp(self.instance_type,
                                                                                                   self.instance_name)
                ...
                # 用benchmark算例替换原有depot_xy, node_xy, node_demand
                depot_xy_benchmark, node_xy_benchmark, node_demand_benchmark = read_benchmark_ovrp(self.instance_type,
                                                                                                   self.instance_name)
                depot_xy.copy_(depot_xy_benchmark)
                node_xy.copy_(node_xy_benchmark)
                node_demand.copy_(node_demand_benchmark)
                ...
                depot_xy, node_xy, node_demand, node_serviceTime, node_earlyTW, node_lateTW = read_benchmark_vrptwp(
        ...
        # shape: (batch, 1)
        depot_lateTW = 4.6 * torch.ones(
            size=(self.batch_size, 1))  # the lenght of time windows should be normalized into [0,1] not 4.6
        # shape: (batch, 1)
        depot_servicetime = torch.zeros(size=(self.batch_size, 1))
        # shape: (batch, 1)
        self.depot_node_earlyTW = torch.cat((depot_earlyTW, node_earlyTW),
                                            dim=1)  # [batch, 1] + [batch, problem] 包含仓库和城市时间信息
        # shape: (batch, problem+1)
    ...
    def step(self, selected):
        ...
        # MOCVRP 和 MOVRPTW (需求量大于剩余容量，以及到达城市时迟于时间窗的点都被mask)
        else:
            # Dynamic-2
            ####################################
            if self.attribute_c:
    ...
    def _get_travel_distance(self):
        if self.attribute_tsp:
            ...
            if self.benchmark_instance:
                ...
                seq_expanded = self.tsp_problems[:, None, :, :].expand(self.batch_size, self.pomo_size,
                                                                       self.problem_size, 4)
        ...
        if self.attribute_mixedtsp:
            ...
            seq_expanded = self.tsp_problems[:, None, :, :].expand(self.batch_size, self.pomo_size, self.problem_size,
                                                                   4)
        ...
        if self.attribute_c:
            ...
            if self.benchmark_instance:
                ...
                depot_xy, node_xy, node_demand = read_benchmark_cvrp(self.instance_type, self.instance_name,
                                                                     normalize=False)
        ...
        if self.attribute_tw:
            ...
            if self.benchmark_instance:
                ...
                # 用benchmark算例替换原有tsp_problems
                depot_node_xy = copy.deepcopy(self.depot_node_xy)
                depot_xy, node_xy, node_demand = read_benchmark_vrptwp(self.instance_type, self.instance_name,
                                                                       normalize=False)
                temp = torch.cat((depot_xy, node_xy), dim=1)
                depot_node_xy.copy_(temp)
                all_xy = depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
        ...
        if self.attribute_ovrp:
            ...
            if self.benchmark_instance:
                ...
                # 用benchmark算例替换原有tsp_problems
                depot_node_xy = copy.deepcopy(self.depot_node_xy)
                depot_xy, node_xy, node_demand = read_benchmark_ovrp(self.instance_type, self.instance_name,
                                                                     normalize=False)
                temp = torch.cat((depot_xy, node_xy), dim=1)
                depot_node_xy.copy_(temp)
                all_xy = depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
...
def read_benchmark_tsp(NAME, num_nodes, normalize=True):
    '''
    NAME = 'Random'
    NAME = 'KroAB'
    '''
...
def read_benchmark_cvrp(NAME, instance_name, normalize=True):
    '''
    NAME = 'A', 'P', 'B', 'E', 'F', 'M', 'X'
    '''
