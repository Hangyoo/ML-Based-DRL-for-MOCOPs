...
from Meta_Transformer_2D_reptile.VRProblemDef import augment_xy_data_by_64_fold
...
class VRPTester:
    ...
    def run(self, shared_problem, pref):
        ...
        while episode < test_num_episode:
            ...
            score_AM_obj1.update(aug_score[0], batch_size)
            score_AM_obj2.update(aug_score[1], batch_size)
            ...
            self.logger.info("episode {:3d}/{:3d}, Elapsed[{}], Remain[{}], Obj1 Score: {:.3f}, Obj2 Score: {:.3f}".format(
                episode, test_num_episode, elapsed_time_str, remain_time_str, score_AM_obj1.avg, score_AM_obj2.avg))

            all_done = (episode == test_num_episode)

            if all_done:
                self.logger.info(" *** Test Done *** ")
                self.logger.info(" NO-AUG f1 SCORE: {:.4f} ".format(score_AM_obj1.avg))
                self.logger.info(" NO-AUG f2 SCORE: {:.4f} ".format(score_AM_obj2.avg))
                # self.logger.info(" NO-AUG SCORE: {:.4f} ".format(score_AM.avg))
                # self.logger.info(" AUGMENTATION SCORE: {:.4f} ".format(aug_score_AM.avg))

        return [score_AM_obj1.avg.cpu(), score_AM_obj2.avg.cpu()]
    ...
    def _test_one_batch(self, shared_probelm, pref, batch_size, episode):
        ...
        self.env.problems = shared_probelm[episode: episode + batch_size]  # self.env.problems (1,200,4)

        if aug_factor == 64:
            self.env.batch_size = self.env.batch_size * 64
            self.env.problems = augment_xy_data_by_64_fold(self.env.problems)  # 对问题进行增强

        self.env.BATCH_IDX = torch.arange(self.env.batch_size)[:, None].expand(self.env.batch_size,
                                                                               self.env.pomo_size)  # (batch_size,pomo_size)
        ...
        self.model.eval()
        with torch.no_grad():
            self.env.load_problems(batch_size, current_problem_type=self.env_params['problem_type'],aug_factor=aug_factor)
            # pref = torch.rand([2])
            # pref = pref / torch.sum(pref)
            reset_state, _, _ = self.env.reset()

            if self.env.attribute_c:
                self.model.decoder_CVRP.assign(pref)
            elif self.env.attribute_tw:
                self.model.decoder_VRPTW.assign(pref)
            elif self.env.attribute_kp:
                self.model.decoder_KP.assign(pref)
            elif self.env.attribute_tsp:
                self.model.decoder_TSP.assign(pref)
            else:
    ...
    def _plot_CVRP(self,nodesCoordinate,depotCoordinate,demands,earlytw,latetw,servicetime,result):
