...
instances = os.listdir(rf'C:\Users\Hangyu\Desktop\CVRP\{type}')
...
for ins in instances:
    try:
        if ins.split('.')[1] == 'sol':
            sol_file = rf'C:\Users\Hangyu\Desktop\CVRP\{type}' + f'\{ins}'
            sol_file_list.append(sol_file)
            insname.append(ins.split('.')[0])
        if ins.split('.')[1] == 'vrp':
            vrp_file = rf'C:\Users\Hangyu\Desktop\CVRP\{type}' + f'\{ins}'
...
for i in range(len(sol_file_list)):
    ...
    f = f'D:\informs\EMNH\Meta_Transformer_2D_single\POMO_MOCVRP/result/tch_aug\cvrp_real/{type}/{insname[i]}_POMO_CVRP.txt'
    data = np.loadtxt(f)
    ...
    save_path = rf'C:\Users\Hangyu\Desktop\GitHub\Experimental Results\MOCVRP\{type}/' + insname[i] + '.txt'
