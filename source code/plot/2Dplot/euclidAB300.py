...
def front_plot(instance_name:str, val=0):
    ...
    exact_data = np.loadtxt(r"D:\informs\EMNH\revised_r2_plot\2Dplot\pf\EuclidAB300.txt")
    ...
    # PDA
    data = np.loadtxt(r"D:\informs\EMNH\plot_revised_paper\PDA_{}_pf.txt".format(instance_name))
    # data = trans(data, f1_min, f1_max, f2_min, f2_max)
    # data = Non_donminated_sorting(data)
    data = trans(data, f1_min, f1_max, f2_min, f2_max)
    data = data[data[:, 0].argsort()]
    ...
    # 局部显示并且进行连线
    zone_and_linked(axes, axins, 110000, 145000, [i for i in range(200000)],
