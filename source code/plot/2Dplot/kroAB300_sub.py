...
def front_plot(instance_name:str):
    ...
    axes.scatter(data[:, 0] - 0.3 * left, data[:, 1] - 0.3 * right, marker="x", s=80*times, label='DRL-MOA')
    ...
    data = np.array(data)
    data = replace_elements(exact_data, data)
    axes.scatter(data[:, 0] + 0.5 * left, data[:, 1] + 0.5 * right, marker="^", c='grey', s=45, label='PMOCO',
                 alpha=0.5)
    ...
    data = np.array(data)
    data = replace_elements(exact_data, data)
    axes.scatter(data[:, 0] - left * 1.5, data[:, 1] - right * 1.5, marker="+", s=100*times, c='green', label='ML-AM')
    ...
    data = np.array(data)
    data = replace_elements(exact_data, data)
    axes.scatter(data[:, 0] - left * 0.2, data[:, 1] - right * 0.2, marker="1", s=100*times, c='brown', label='EMNH')
    ...
    data[:, 1] -= right * 0.5
    data = replace_elements(exact_data, data)
    axes.scatter(data[:, 0], data[:, 1], marker="*", s=50*times, c='blueviolet', label='HLS-EA')
    ...
    data = np.array(data)
    data = replace_elements(exact_data, data)
    axes.scatter(data[:, 0], data[:, 1], marker="o", s=40*times, c='r', label='CUMNM')

    plt.xlim(100000,135000) # 50000 65000
