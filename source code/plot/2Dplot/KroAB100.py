...
def front_plot(instance_name:str, val=0):
    ...
    exact_data = np.loadtxt(r"D:\informs\EMNH\revised_r2_plot\2Dplot\pf\KroAB300.txt")
    ...
    axes.scatter(data[:,0]-0.3*left,data[:,1]-0.3*right, marker="x", s=40,label = 'DRL-MOA')
    ...
    data = np.array(data)
    data = replace_elements(exact_data,data)
    axes.scatter(data[:,0]+0.5*left,data[:,1]+0.5*right, marker="^", c='grey', s=45,label = 'PMOCO', alpha=0.5)
    ...
    data = np.array(data)
    data = replace_elements(exact_data,data)
    axes.scatter(data[:,0]-left*1.5,data[:,1]-right*1.5, marker="+", s=40, c='green', label = 'ML-AM')
    ...
    data = np.array(data)
    data = replace_elements(exact_data,data)
    axes.scatter(data[:,0]-left*0.2,data[:,1]-right*0.2, marker="1", s=50, c='brown', label = 'EMNH')
    ...
    # Proposed over
    data = np.loadtxt(r"C:\Users\Hangyu\Desktop\GitHub\Experimental Results\MOTSP\KroAB300.txt")
    # data = trans(data, f1_min, f1_max, f2_min, f2_max)
    data = Non_donminated_sorting(data)
    data = np.array(data)
    data = replace_elements(exact_data,data)
    ...
    # 局部显示并且进行连线
    zone_and_linked(axes, axins, 100000, 135000, [i for i in range(200000)],
...
if __name__ == '__main__':
    ...
    # instance_name = 'KroAB300'
    # instance_name = 'Euclid100'
    # instance_name = 'Euclid300'
