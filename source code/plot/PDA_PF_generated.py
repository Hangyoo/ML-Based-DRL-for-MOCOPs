...
def generatePF(instance_name):
    data = np.loadtxt(r"E:\Paper Data\paper 3\PF\{}.txt".format(instance_name))
    # 找到真实前沿的最大值
    ...
    for i in range(new_data.shape[0]):
        if new_data[i][0] >= x_max*0.9 or new_data[i][1] >= y_max*0.9:
            pass
        else:
    ...
    np.savetxt('./PDA_{}_pf.txt'.format(instance_name),repair_data)
