...
from MOKP.MOKProblemDef_5obj import get_random_problems
...
class KPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        # MOKP
        ###################################
        self.items_and_a_dummy = torch.Tensor(np.zeros((self.batch_size, self.problem_size+1, 6)))
    ...
    def step(self, selected):
        ...
        # Status
        ####################################
        items_mat = self.items_and_a_dummy[:, None, :, :].expand(self.batch_size, self.pomo_size, self.problem_size+1, 6)
        gathering_index = selected[:, :, None, None].expand(self.batch_size, self.pomo_size, 1, 6)
        ...
        reward = None
        if done:
            reward = torch.stack([self.accumulated_value_obj1,self.accumulated_value_obj2,self.accumulated_value_obj3,self.accumulated_value_obj4,self.accumulated_value_obj5],axis = 2)
        else:
            reward = None
