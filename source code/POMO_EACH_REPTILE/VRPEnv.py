...
from Meta_Transformer_2D_reptile.VRProblemDef import get_random_problems_mixed, augment_xy_data_by_8_fold, augment_xy_data_by_64_fold
...
class VRPEnv:
    ...
    def load_problems(self, batch_size, current_problem_type, aug_factor=1):
        ...
        if not self.FLAG__use_saved_problems:
            ...
            depot_xy, node_xy, node_demand, node_earlyTW, node_lateTW, node_servicetime, tsp_problems, kp_problems, type = get_random_problems_mixed(batch_size, self.problem_size, current_problem_type)
        else:
        ...
        if aug_factor > 1:
            if aug_factor == 8:
                ...
                tsp_problems = augment_xy_data_by_8_fold(tsp_problems)
                node_demand = node_demand.repeat(8, 1)
                node_earlyTW= node_earlyTW.repeat(8,1)
                node_lateTW = node_lateTW.repeat(8,1)
                kp_problems = kp_problems.repeat(8,1)
