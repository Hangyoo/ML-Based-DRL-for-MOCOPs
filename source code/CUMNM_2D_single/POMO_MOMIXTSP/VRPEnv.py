    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if not self.FLAG__use_saved_problems and (self.benchmark_instance == False):
            ...
            if self.problem_type == 'MOTSP':
                ...
                # 用benchmark算例替换原有tsp_problems
                tsp_data = read_benchmark_mixedtsp(self.instance_type, self.problem_size)
                tsp_problems.copy_(tsp_data)
                print(self.batch_size, self.problem_size, self.problem_type)
            elif self.problem_type == 'MOCVRP':
                # 用benchmark算例替换原有depot_xy, node_xy, node_demand
                depot_xy_benchmark, node_xy_benchmark, node_demand_benchmark = read_benchmark_cvrp(self.instance_type, self.instance_name)
    ...
    def _get_travel_distance(self):
        ...
        if self.attribute_mixedtsp:
            ...
            if self.benchmark_instance:
                ...
                # 用benchmark算例替换原有tsp_problems
                tsp_problems = copy.deepcopy(self.tsp_problems)
                tsp_data = read_benchmark_mixedtsp(self.instance_type, self.problem_size, normalize=False)
                tsp_problems.copy_(tsp_data)
                seq_expanded = tsp_problems[:, None, :, :].expand(self.batch_size, self.pomo_size, self.problem_size, 4)
            else:
                seq_expanded = self.tsp_problems[:, None, :, :].expand(self.batch_size, self.pomo_size, self.problem_size, 4)
...
def read_benchmark_mixedtsp(NAME, num_nodes, normalize=True):

    path = r'D:\Paper Data\paper 3\instance\MIXTSP_Benchmark'
    x1 = np.loadtxt(path + f'/euclidG{num_nodes}.tsp', skiprows=6, usecols=(1, 2), delimiter=' ', dtype=float).reshape(-1,2)
    x2 = np.loadtxt(path + f'/mixed{num_nodes}.txt', dtype=float).reshape(-1,1)
    x3 = np.zeros(shape=(num_nodes,1))

    if normalize:
        x1 = x1 / (np.max(x1, 0))  # 数据归一化
        x2 = x2 / np.max(x2)  # 数据归一化

    tsp_data = np.concatenate((x1, x2, x3), axis=1)  # (100,2) --> (100,4) 改变了第1维度, 即axis=1
    tsp_data = tsp_data.reshape(1, num_nodes, 4)
    tsp_data = torch.tensor(tsp_data, device='cuda')
