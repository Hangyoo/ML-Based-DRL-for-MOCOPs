...
item = 100  # 100 300
ty = 'MixedAB'
modelpath = r'D:\Paper Data\paper 3\R2\POMO_MOMIXTSP\result\20240910_173401_train_MOMIXTSP_n50_with_instNorm'
# epoch = 2000
for item in [100,300,500]:
    for epoch in range(10,2010,10):
        problem_size = item

        env_params = {
            'problem_type': "MOMIXTSP", # test problem type
        ...
        def main(n_sols = 101,idx=None):
            ...
            np.savetxt(f"result/{subfile}/mixedtsp_real/{item}/{env_params['instance_type']}{problem_size}_epoch_{epoch}_real.txt", sols)
            with open(f"result/{subfile}/mixedtsp_real/{item}/TIME_{env_params['instance_type']}{problem_size}_epoch_{epoch}_real.txt", 'w') as f:
            ...
            plt.savefig(f'result/{subfile}/mixedtsp_real/{item}/{ty}{item}_epoch_{epoch}_real.png')
            plt.clf()
            # plt.show()
