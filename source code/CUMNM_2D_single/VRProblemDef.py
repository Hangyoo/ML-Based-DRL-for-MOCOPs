...
def get_random_problems_mixed(batch_size, problem_size, problem_type):
    ...
    # < 0.25 是为CVRP问题
    if ((problem_type == 'unified' and seed < 0.2) or 'C' in problem_type):  # MOCVRP
    ...
    if ((problem_type == 'unified' and seed < 0.2) or 'OVRP' in problem_type):  # MOOVRP
    ...
    if ((problem_type == 'unified' and seed>=0.2 and seed <0.4) or 'TW' in problem_type): #VRPTW problem_type is 'unified' or there is 'TW' in the problem_type
    ...
    if ((problem_type == 'unified' and seed>=0.4 and seed <0.6) or 'TSP' in problem_type):  # MOTSP
        tsp_problems = torch.rand(size=(batch_size, problem_size, 4))
        type = 'TSP'

    if ((problem_type == 'unified' and seed>=0.6 and seed <0.8) or 'MIXTSP' in problem_type):  # MOMIXTSP
        tensor1 = torch.rand(size=(batch_size, problem_size, 3))
        tensor2 = torch.zeros(size=(batch_size, problem_size, 1))
        tsp_problems = torch.cat((tensor1, tensor2), dim=2)
        type = 'MIXTSP'

    # if ((problem_type == 'unified' and seed>=0.5 and seed <0.75) or 'O' in problem_type):  # OCVRP
    #     depot_xy = torch.rand(size=(batch_size, 1, 2))
    #     # shape: (batch, 1, 2)
    #
    #     node_xy = torch.rand(size=(batch_size, problem_size, 2))
    ...
    if ((problem_type == 'unified' and seed>=0.8) or 'KP' in problem_type): #MOKP
        if problem_size == 50:
...
def augment_xy_data_by_64_fold(xy_data):
    ...
    aug_xy_data = torch.cat((dat1, dat2, dat3, dat4, dat5, dat6, dat7, dat8), dim=0)
    aug_xy_data = aug_xy_data.repeat(8,1,1)
    # shape: (64*batch, N, 2)

    return aug_xy_data

def augment_xy_data_by_64_fold_tsp(xy_data):
    x1 = xy_data[:, :, [0]]
    y1 = xy_data[:, :, [1]]
    x2 = xy_data[:, :, [2]]
    y2 = xy_data[:, :, [3]]
...
def augment_xy_data_by_8_fold_tsp(xy_data):
    x1 = xy_data[:, :, [0]]
    y1 = xy_data[:, :, [1]]
    x2 = xy_data[:, :, [2]]
    y2 = xy_data[:, :, [3]]
    ...
    # 从原始列表中随机选择 8 个不重复的元素
    dat_aug = random.sample(dat_aug, 8)
