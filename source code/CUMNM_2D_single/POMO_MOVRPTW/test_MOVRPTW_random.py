...
modelpath = r'D:\informs\EMNH\Meta_Transformer_2D_single\POMO_MOVRPTW'
...
for instance_name in [f for f in os.listdir(r'C:\Users\Hangyu\Desktop\GitHub\0')]:
    print(f'当前执行{instance_name}')
    problem_size = 100

    env_params = {
        'problem_type': "MOVRPTW", # test problem type
    ...
    tester_params = {
        ...
        'model_load': {
            'path': modelpath,  # directory path of pre-trained model and log files saved.
    ...
    def main(n_sols = 101,idx=None):
        ...
        # 保存结果到.txt格式
        from Meta_Transformer_2D_single.Nondominate_sort import Non_donminated_sorting
        sols = Non_donminated_sorting(sols)
        sols = np.array(sols)
        # np.savetxt(fr"C:\Users\Hangyu\Desktop\GitHub\Experimental Results\Random Test Set\MOVRPTW\{instance_name}", sols)
        np.savetxt(fr"C:\Users\Hangyu\Desktop\GitHub\Experimental Results\MOVRPTW\{instance_name}", sols)
        # subfile = 'tch_aug' if env_params['aggregate_function'] == 'TCH' else 'wei_aug'
        # np.savetxt(f"result/{subfile}/vrptw_real/{instance_name}_each_decoder.txt", sols)
        # with open(f"result/{subfile}/vrptw_real/TIME_{instance_name}_each_decoder.txt", 'w') as f:
        ...
        # plt.savefig(f'result/{subfile}/vrptw_real/{instance_name}_each_decoder.png')
        # plt.show()
        # plt.clf()
