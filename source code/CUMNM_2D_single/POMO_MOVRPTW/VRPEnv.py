    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if not self.FLAG__use_saved_problems and (self.benchmark_instance == False):
            ...
            if self.problem_type == 'MOTSP':
                ...
                depot_xy, node_xy, node_demand, node_serviceTime, node_earlyTW, node_lateTW = read_benchmark_vrptwp(self.instance_name)
    ...
    def _get_travel_distance(self):
        ...
        if self.attribute_tw:
            ...
            if self.benchmark_instance:
                ...
                # 用benchmark算例替换原有tsp_problems
                depot_node_xy = copy.deepcopy(self.depot_node_xy)
                depot_xy, node_xy, node_demand, node_serviceTime, node_earlyTW, node_lateTW = read_benchmark_vrptwp(self.instance_name, normalize=False)
                temp = torch.cat((depot_xy, node_xy), dim=1)
                depot_node_xy.copy_(temp)
                all_xy = depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
            else:
...
def read_benchmark_vrptwp(instance_name, normalize=True, random=True):
    ...
    if not random:
        ...
        path = r'C:\Users\Hangyu\Desktop\GitHub\0/'
        data = np.loadtxt(path + instance_name, skiprows=5)
        # path = r'C:\Users\Hangyu\Desktop\VRPTW-Solomon(56)\normal/'
        # data = np.loadtxt(path + instance_name)
        depot_xy = data[0, :2]
        node_xy = data[1:, :2]
        node_demand = data[1:,2]
        node_serviceTime = data[1:,5]
        node_earlyTW = data[1:,3]
        node_lateTW = data[1:,4]
