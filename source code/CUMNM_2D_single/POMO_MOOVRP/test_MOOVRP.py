...
modelpath = r'D:\Paper Data\paper 3\R2\POMO_MOOVRP\result\20240910_210916_train_MOOVRP_n50_with_instNorm'

ty = 'C'  # A P B E F M X
problem_size = 50
instance_name = 'CMT1.csv'
...
for epoch in range(10,2010,10):
    ...
    env_params = {
        'problem_type': "MOOVRP", # test problem type
    ...
    def main(n_sols = 101,idx=None):
        ...
        np.savetxt(f"result/{subfile}/cvrp_real/{ty}/{env_params['instance_type']}{instance_name.split('.')[0][3:]}_epoch_{epoch}.txt",sols)
        with open(f"result/{subfile}/cvrp_real/{ty}/TIME_{env_params['instance_type']}{instance_name.split('.')[0][3:]}_epoch_{epoch}.txt",'w') as f:
        ...
        plt.savefig(f'result/{subfile}/cvrp_real/{ty}/{instance_name}_epoch_{epoch}.png')
        plt.clf()
        # plt.show()
