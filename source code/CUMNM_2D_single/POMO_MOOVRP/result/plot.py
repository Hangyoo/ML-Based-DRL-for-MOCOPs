...
# DRL-MOA over
data = np.loadtxt(r"D:\informs\EMNH\Meta_Transformer_2D_single\POMO_MOOVRP\result\tch_noaug\cvrp_real\C\C1_epoch_10.txt")
data = Non_donminated_sorting(data)
data1 = np.array(data)
axes.scatter(data1[:,0],data1[:,1], marker="v", s=50, color='darkmagenta', label = '10')

# LI-Transfer  over
data = np.loadtxt(r"D:\informs\EMNH\Meta_Transformer_2D_single\POMO_MOOVRP\result\tch_noaug\cvrp_real\C\C1_epoch_2000.txt")
data = Non_donminated_sorting(data)
data = np.array(data)
axes.scatter(data[:,0],data[:,1], marker="x", s=80,label = '2000')
