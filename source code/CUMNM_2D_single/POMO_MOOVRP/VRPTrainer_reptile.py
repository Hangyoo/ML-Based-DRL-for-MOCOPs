...
from Meta_Transformer_2D_single.utils.utils import *
...
class VRPTrainer:
    def __init__(self,
        ...
        if model_load['enable']:
            ...
            if not trainer_params['finetune_step']:
                # 不微调的情况下使用的, 接着进行训练
    ...
    def finetune(self, weight=None, model=None, finetuneType=None):
        if weight is None:
            weight = torch.tensor([0.5, 0.5])
        if model is None:
            fine_model = copy.deepcopy(self.model)
        else:
            fine_model = copy.deepcopy(model)
        print("--------------------------------------------")
        print("weight:{}, {}".format(weight[0].item(), weight[1].item()))
        print("finetuning meta_model...")
        batch_size = self.trainer_params['train_batch_size']
        finetune_step = self.trainer_params['finetune_step']
        for step in range(finetune_step):
            avg_score_obj1, avg_score_obj2, avg_loss = self._finetune_one_batch(weight, fine_model, batch_size, finetuneType)
            if (step+1) % 100 == 0:
                print('finetune_step:{}, obj1:{}, obj2:{}, loss:{}'.format(step+1, avg_score_obj1, avg_score_obj2, avg_loss))

        test_save_dir = self.trainer_params['model_load']['path'] + '_finetune'
        if os.path.exists(test_save_dir):
            pass
        else:
            os.makedirs(test_save_dir)
        # Save Model
        epoch = self.trainer_params['model_load']['epoch']
        checkpoint_dict = {
            'epoch': epoch,
            'model_state_dict': get_inner_model(self.model).state_dict(),
            'optimizer_state_dict': self.optimizer.state_dict(),
        ...
        torch.save(checkpoint_dict, '{}/checkpoint-{}.pt'.format(test_save_dir, epoch))

        return fine_model

    def validate(self, weight=None, model=None, finetuneType=None):
        if weight is None:
            weight = torch.tensor([0.5, 0.5])
        if model is None:
        ...
        finetune_step = self.trainer_params['finetune_step']
        for step in range(finetune_step):
            avg_score_obj1, avg_score_obj2 = self._validate_one_batch(weight, valid_model, batch_size, finetuneType)
            print('finetune_step:{}, obj1:{}, obj2:{}'.format(step + 1, avg_score_obj1, avg_score_obj2))
    ...
    def _train_one_batch(self, batch_size):
        ...
        if self.env.attribute_c:
            ...
            self.sub_model.decoder_OVRP.assign(pref)
